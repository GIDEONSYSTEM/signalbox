...
private:
    ...
    bool refreshTimecode();            // таймкод одним REST-запросом; false = камера не ответила
    void markLost(const char* why);    // пометить офлайн и погасить подписку
    ...
    // 🔴 Сколько подряд неудачных чтений считаем потерей связи. Нужно потому,
    // что оборванная подписка НЕ всегда закрывает сокет: если камеру выключили
    // или у неё отключили управление, recvText просто молчит по таймауту, и без
    // этой проверки камера навсегда осталась бы в панели «рабочей» с застывшими
    // значениями.
    static constexpr int kLostAfterFails = 3;
    int                m_tcFailStreak = 0;
