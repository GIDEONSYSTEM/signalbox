...
// Короткая метка ОС: "win", "mac". Нужна ровно там, где решение принимает
// прикладной код, а не слой, — сейчас это выбор архива в релизе (в одном
// релизе лежат сборки под обе ОС, и скачать надо свою).
// Значение попадает в ИМЕНА ФАЙЛОВ релиза, поэтому менять его нельзя:
// старые версии перестанут находить своё обновление. Новая ОС — новая метка.
...
// Местное время «ЧЧ:ММ:СС» — для событий, которые потом разбирают по логу
// (перегрев, обрывы). Функции времени в стандартной библиотеке различаются
// потокобезопасными вариантами на разных ОС, поэтому живут здесь.
...
// Работают, даже когда панель не в фокусе — как в OBS.
//
// 🔴 Регистрируем КОНКРЕТНЫЕ комбинации у системы, а НЕ вешаем низкоуровневый
// хук клавиатуры. Хук видит вообще всё нажатое в системе: именно на это ругаются
// антивирусы, и именно он требует на macOS разрешения «Мониторинг ввода».
// Регистрация чужой ввод не читает и разрешений не просит. Плата: пока
// программа запущена, комбинация занята и до других приложений не доходит —
// поэтому в панели предупреждаем и советуем клавиши с модификаторами или F13+.
struct HotKey {
    std::string key;                 // каноническое имя: "F5", "A", "1", "Space", "Numpad1"
    bool ctrl = false, alt = false, shift = false, meta = false;   // meta = Win / Cmd
};

bool hotkeysSupported();
// Кто получит уведомление о нажатии. Вызывается из потока цикла событий.
void onHotkey(std::function<void(int id)> handler);
// Заменяет ВЕСЬ набор зарегистрированных клавиш на новый (проще и надёжнее,
// чем добавлять и снимать по одной: набор меняется целиком при правке групп).
// Возвращает список id, которые система заняла отказом — их панель покажет
// как конфликт.
