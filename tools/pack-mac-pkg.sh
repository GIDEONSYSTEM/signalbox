...
# 🔴 res/license.txt — РАБОЧИЙ ЧЕРНОВИК, юристом не смотренный. Написан под этот
# продукт (камеры, запись, SDK Sony), а не общий шаблон, но перед первой продажей
# его обязан прочитать юрист. Два места прямо помечены как незаполненные:
# юрисдикция и контакт — их за владельца выдумывать нельзя.
