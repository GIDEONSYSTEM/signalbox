...
// только один случай из девятнадцати (сейчас там один архив без метки), а
...
int main() {
    ...
    // 🔴 ОБРАТНАЯ СТОРОНА того же правила, найдена на ревью с Windows-машины:
    // метка засчитывается, только если сразу за ней НЕ буква и НЕ цифра. Значит
    // «SignalBox-1.0.6-win64.zip» меткой не считается, уходит в «без метки» —
    // и достаётся не только Windows, но и Mac. Релиз при этом выглядит нормально.
    // Тест закрепляет поведение КАК ЕСТЬ, чтобы оно не менялось молча: лечится
    // не кодом, а именем файла — ровно «-win.zip» / «-mac.zip» (§7).
    check("win64 меткой НЕ считается и уедет на mac",
          assets("SignalBox-1.0.6-win64.zip"), "mac", "SignalBox-1.0.6-win64.zip", true);
    check("а рядом настоящий mac-архив победит",
          assets("SignalBox-1.0.6-win64.zip,SignalBox-1.0.6-mac.zip"),
          "mac", "SignalBox-1.0.6-mac.zip", true);
